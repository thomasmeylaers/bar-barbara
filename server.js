...
function payed(request, response) {
    ...
    var name = body['name'];
    var clientDrinks = bills[name]['drinks'];
    drinks.forEach(drink => {
        console.log(drink);
        if(clientDrinks.includes(drink)){
            
            clientDrinks.splice(clientDrinks.indexOf(drink),1);
        }
    });
    var parsingData=JSON.stringify(bills);
    fs.writeFile('bills.json', parsingData, finished);
    function finished(){
        response.send(clientDrinks);
    }
